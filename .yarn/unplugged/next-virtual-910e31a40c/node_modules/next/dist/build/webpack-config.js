...
var _forkTsCheckerWebpackPlugin = _interopRequireDefault(
  require("fork-ts-checker-webpack-plugin")
);
var _constants = require("next-server/constants");
var _index = _interopRequireDefault(
  require("next/dist/compiled/resolve/index.js")
);
...
var _buildManifestPlugin = _interopRequireDefault(
  require("./webpack/plugins/build-manifest-plugin")
);
var _chunkGraphPlugin = require("./webpack/plugins/chunk-graph-plugin");
var _chunkNamesPlugin = _interopRequireDefault(
  require("./webpack/plugins/chunk-names-plugin")
);
...
var _nextEsmPlugin = _interopRequireDefault(
  require("./webpack/plugins/next-esm-plugin")
);
var _nextjsSsrImport = _interopRequireDefault(
  require("./webpack/plugins/nextjs-ssr-import")
);
var _nextjsSsrModuleCache = _interopRequireDefault(
  require("./webpack/plugins/nextjs-ssr-module-cache")
);
var _pagesManifestPlugin = _interopRequireDefault(
...
var _index2 = require("./webpack/plugins/terser-webpack-plugin/src/index");
...
async function getBaseWebpackConfig(
  ...
  const outputDir = isLikeServerless
    ...
    ? {
        ...
        [_constants.CLIENT_STATIC_FILES_RUNTIME_MAIN]:
          ...
          _path.default.relative(
            dir,
            _path.default.join(
              _constants2.NEXT_PROJECT_ROOT_DIST_CLIENT,
  ...
  let typeScriptPath;
  try {
    typeScriptPath = _index.default.sync("typescript", { basedir: dir });
  } catch (_) {}
  const tsConfigPath = _path.default.join(dir, "tsconfig.json");
  const useTypeScript = Boolean(
    typeScriptPath && (await (0, _fileExists.fileExists)(tsConfigPath))
  );
  const resolveConfig = {
    ...
    alias: {
      ...
      next: _constants2.NEXT_PROJECT_ROOT,
      [_constants2.PAGES_DIR_ALIAS]: _path.default.join(dir, "pages"),
      [_constants2.DOT_NEXT_ALIAS]: distDir
    },
  ...
  const devtool = dev ? "cheap-module-source-map" : false; // Contains various versions of the Webpack SplitChunksPlugin used in different build types
  const splitChunksConfigs = {
    ...
    prodGranular: {
      ...
      cacheGroups: {
        ...
        commons: { name: "commons", minChunks: totalPages, priority: 20 },
        shared: {
          name(module, chunks) {
            return _crypto.default
              .createHash("sha1")
  ...
  let webpackConfig = {
    ...
    optimization: {
      ...
      minimizer: [
        new _index2.TerserPlugin({ ...terserPluginConfig, terserOptions })
      ]
    },
    recordsPath: _path.default.join(outputPath, "records.json"),
    context: dir, // Kept as function to be backwards compatible
    entry: async () => {
      return { ...(clientEntries ? clientEntries : {}), ...entrypoints };
    },
    output: {
      path: outputPath,
      filename: ({ chunk }) => {
        ...
        if (
          !dev &&
          (chunk.name === _constants.CLIENT_STATIC_FILES_RUNTIME_MAIN ||
            chunk.name === _constants.CLIENT_STATIC_FILES_RUNTIME_WEBPACK)
        ) {
    ...
    resolveLoader: {
      alias: {
        ...
        "emit-file-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "emit-file-loader"
        ),
        "next-babel-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "next-babel-loader"
        ),
        "next-client-pages-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "next-client-pages-loader"
        ),
        "next-data-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "next-data-loader"
        ),
        "next-serverless-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "next-serverless-loader"
        ),
        "noop-loader": _path.default.join(
          __dirname,
          "webpack",
          "loaders",
          "noop-loader"
    ...
    plugins: [
      ...
      new _chunkGraphPlugin.ChunkGraphPlugin(buildId, {
        dir,
        distDir,
        isServer
      ...
      config.future.excludeDefaultMomentLocales &&
        new _webpack.default.IgnorePlugin(/^\.\/locale$/, /moment$/),
      ...(dev
        ? (() => {
        ...
        : []),
      !dev && new _webpack.default.HashedModuleIdsPlugin(),
      !dev &&
      ...
      isServerless && new _serverlessPlugin.ServerlessPlugin(),
      isServer && new _pagesManifestPlugin.default(isLikeServerless),
      target === "server" &&
        isServer &&
        new _nextjsSsrModuleCache.default({ outputPath }),
      ...
      tracer && new _profilingPlugin.ProfilingPlugin({ tracer }),
      !isServer &&
        useTypeScript &&
    ...
    webpackConfig.entry = async () => {
      const entry =
        typeof originalEntry === "function"
          ? await originalEntry()
          : originalEntry; // Server compilation doesn't have main.js
      if (clientEntries && entry["main.js"] && entry["main.js"].length > 0) {
        const originalFile =
          clientEntries[_constants.CLIENT_STATIC_FILES_RUNTIME_MAIN];
        entry[_constants.CLIENT_STATIC_FILES_RUNTIME_MAIN] = [
          ...entry["main.js"],
          originalFile
        ];
      }
