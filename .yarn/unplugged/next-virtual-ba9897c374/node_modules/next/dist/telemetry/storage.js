...
const TELEMETRY_KEY_SALT=`telemetry.salt`;const{NEXT_TELEMETRY_DISABLED,NEXT_TELEMETRY_DEBUG}=process.env;let isDisabled=true||!!NEXT_TELEMETRY_DISABLED;function notify(){// No notification needed if telemetry is not enabled
...
if(isDisabled){return Promise.resolve();}const anonymousId=config.get(TELEMETRY_KEY_ID);if(!anonymousId){return Promise.resolve();}const context={anonymousId:anonymousId,projectId:projectId,sessionId:randomRunId};return(0,_postPayload2._postPayload)(`https://telemetry.nextjs.org/api/v1/record`,{context,events:events.map(({eventName,payload})=>({eventName,fields:payload}))});}function record(_events){// pseudo try-catch
